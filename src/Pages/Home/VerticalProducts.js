...
const VerticalProducts = ({ category, title }) => {
    const [data, setData] = useState([]);

    const containerRef = useRef(null);

    //console.log("horizental", category);
        ...
        <>
            <div className=' container flex '>
                <div className='verticaldev '>
                    ...
                    <div style={{ marginTop: "5px", }} ref={containerRef}>
                        <div className='flex flex-wrap  justify-center ' >
                            {data ? (data.map((product, index) => (
                                <div key={index} className='cardSection'>
                                    <Link to={`/detailitem/${product._id}/${product.category}`}>
                                        <div className="card m-1 lg:w-1/2 " style={{ width: "15rem", height: "16rem", border: "1px solid gray" }}>
                ...
                <div className=' lg:w-1/2  '>
                    <img className="card-img-top h-full" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSkdMISRlNH-6YHcCtAMlWQwRf9R1WdG0onfQ&s" alt="Card image cap" />
